...
make_qwi_tsibble <- function(df){
  ...
  if("metropolitan statistical area/micropolitan statistical area" %in% col_names){
    ...
    tsibble::as_tsibble(df, key = id(`metropolitan statistical area/micropolitan statistical area`| state, agegrp, ownercode, sex, industry),
               index = year_time)
  } else{
    message("Using County")
    #id_var <- "county | state state, agegrp, ownercode, sex, industry"

    tsibble::as_tsibble(df, key = id(county| state, agegrp, ownercode, sex, industry),
               index = year_time)
